...
def index(request, p_index=1):
    ...
    context = {"product_list": list2, 'plist': plist, 'p_index': p_index, 'max-pages': maxpages, 'mywhere': mywhere}
...
def edit(request, cid=0):
    ...
    try:
        ...
        slist = slist.filter(status__lt=9)
        context["shoplist"] = slist
        catlist = Category.objects.values("id", 'name')
        catlist = catlist.filter(status__lt=9)
        context["categorylist"] = catlist
